...
3. Within the cloned repo, extract the contents of the zip into its own folder sample-bulk-fhir-datasets-1000-patients/.
...
5. Run load.py in Terminal: `python 3 load.py`. This will load all the JSONs into raw tables.
6. Configure `~/.dbt/profiles.yml` to point to the local `novellia.duckdb` file. Sample file located at `profiles.yml`.
7. Run/build the dbt staging models while you are in the novellia/ dbt project folder. Run: `cd novellia` to get to the right directory. Then run `dbt build --select staging`.
